...
void NodeState::updateLocalCost()
{
    ...
    {
        local_cost = INF_COST;
        // Serial.printf("Node %d is %s — local cost set to INF\n", node_id, !sensors.alive ? "DEAD" : "BLOCKED");
        return;
    }
    ...
    // Serial.printf("Node %d local cost updated: %.2f (Pressure: %.2f, Fire: %.2f, H_air: %.2f, Heat: %.2f)\n",
                //   node_id, local_cost, sensors.pressure_norm, cached_fire_value, cached_h_air, sensors.heat_norm);
}
